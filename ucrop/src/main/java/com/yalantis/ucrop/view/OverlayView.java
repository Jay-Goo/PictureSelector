...
public class OverlayView extends View {
    ...
    public static final float DEFAULT_CROP_GRID_ADJUST_DP = 30f;

    private float DEFAULT_CROP_VIEW_TOP,DEFAULT_CROP_VIEW_BOTTOM,DEFAULT_CROP_VIEW_LEFT,DEFAULT_CROP_VIEW_RIGHT;
    ...
    private float mCropGridAdjustDp;
    ...
    private boolean mIsCropGridAdjust;
    ...
    /**
     * Setter for crop grid rows and columns adjust by the same value.
     * Resets {@link #mGridPoints} variable because it is not valid anymore.
     */
    public void setCropGridAdjust(@FloatRange (from = 0) float dp) {
        if (dp > 0) {
            mIsCropGridAdjust = true;
            mCropGridAdjustDp = dp;
            mGridPoints = null;
        }else {
            mIsCropGridAdjust = false;
        }
    }
    /**
    ...
    public void setupCropBounds() {
        ...
        if (height > mThisHeight) {
            ...
            DEFAULT_CROP_VIEW_TOP = getPaddingTop();
            DEFAULT_CROP_VIEW_LEFT = getPaddingLeft() + halfDiff;
            DEFAULT_CROP_VIEW_RIGHT = getPaddingLeft() + width + halfDiff;
            DEFAULT_CROP_VIEW_BOTTOM = getPaddingTop() + mThisHeight;
        } else {
            int halfDiff = (mThisHeight - height) / 2;
            mCropViewRect.set(getPaddingLeft(), getPaddingTop() + halfDiff,
                    getPaddingLeft() + mThisWidth, getPaddingTop() + height + halfDiff);
            DEFAULT_CROP_VIEW_TOP = getPaddingTop() + halfDiff;
            DEFAULT_CROP_VIEW_LEFT = getPaddingLeft();
            DEFAULT_CROP_VIEW_RIGHT =  getPaddingLeft() + mThisWidth;
            DEFAULT_CROP_VIEW_BOTTOM = getPaddingTop() + height + halfDiff;
        }
    ...
    public void resetGride(){
        mCropViewRect.set(DEFAULT_CROP_VIEW_LEFT,DEFAULT_CROP_VIEW_TOP,DEFAULT_CROP_VIEW_RIGHT,DEFAULT_CROP_VIEW_BOTTOM);
        mTempRect.set(mCropViewRect);
        updateGridPoints();
        postInvalidate();
    }
    ...
    protected void drawCropGrid(@NonNull Canvas canvas) {
        if (mShowCropGrid) {
            if (mGridPoints == null && !mCropViewRect.isEmpty()) {
                ...
                if (mIsCropGridAdjust) {
                    mCropGridRowCount = (int) (mCropViewRect.height() / Utils.convertDpToPixel(mCropGridAdjustDp, mContext));
                    mCropGridColumnCount = (int) (mCropViewRect.width() / Utils.convertDpToPixel(mCropGridAdjustDp, mContext));
                }
